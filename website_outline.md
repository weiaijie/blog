...
- **V1（MVP，优先上线）**
  - 首页、关于我、技能、项目、联系方式
  - 静态内容优先，保证可浏览、可联系、可展示代表作品
- **V2（增强）**
  - 博客模块（列表、详情、标签、搜索）
  - 评论、点赞、阅读量等互动功能（通过第三方服务集成）
- **V3（可选扩展）**
  - 自建后端、CMS、账号体系、复杂数据分析
  - 仅在明确有维护资源时启动
...
- **静态部署 vs 动态功能**
  - 需要服务端能力的功能（评论、点赞、表单处理、阅读量）不得直接假设“自研后端”
  - V1 必须给出可落地替代方案（如 Formspree、EmailJS、Giscus、Plausible/GA）
- **技术选型冲突**
  - 文档中如出现“或/可选”，默认归入“备选方案”，不影响当前主线实施
  - 当前主线以 0.2 的基线为准

### 0.4 模块完成定义（Definition of Done）🔄

- **页面完成判定**
  - 移动端（<=768px）和桌面端（>=1280px）布局均正常
  - 关键交互可用（导航、跳转、表单提交、筛选）
  - 无阻塞性错误（控制台无红色报错）
- **质量判定**
...
- **实现边界（V2默认）**
  - 博客页面仍按静态站方式发布
  - 互动能力优先接入第三方服务，不自建账号系统
  - 如后续需要自建CMS或用户体系，升级到V3执行
...
- **内容管理系统**
  - 以 Git + Markdown 为主流程（本地编辑 + PR合并发布）
  - 支持草稿、预览、发布、更新
  - 分类和标签通过 front matter 管理
  - 可选：接入轻量化Git CMS（如 Decap CMS）
- **统计和分析**
  - 页面访问统计（PV/UV）
  - 用户互动数据（评论数、分享行为）
  - 热门内容分析（按时间窗口）
  - 可选：Google Analytics 或 Plausible
...
- **表单字段**
  ...
  - 可选：公司/组织名称
- **表单功能**
  - 输入验证（客户端必做，服务端校验依赖第三方表单服务）
  - 防垃圾邮件措施（如验证码或隐藏字段）
  - 提交成功反馈页面或消息
  - 错误处理和用户友好提示
- **数据处理**
  - 表单提交后的数据处理流程（第三方服务转发到邮箱）
  - 自动回复邮件确认（可选）
  - 数据存储和管理方式（明确存储位置与访问权限）
  - 隐私政策说明（用途、保留时长、删除方式）
...
- **前端框架**
  - React.js生态系统（默认）
  - 使用函数组件和Hooks
  - 状态管理（Context API，复杂场景再引入Redux）
  - 路由管理（React Router）
  - 代码分割和懒加载
  - Vue.js作为备选方案，当前版本不采用
- **构建工具和流程**
  - Vite打包（默认）
  - Babel转译
  - PostCSS处理
