...
export default {
	...
	methods: {
		...
		async checkOrderStatus() {
			try {
				// 使用已有的 getCashierOrder 接口检查订单状态
				const res = await getCashierOrder(this.orderId, this.fromType);
				if (res.data) {
					// 根据返回数据判断订单是否已支付
					// 如果 pay_price 为 0 或者订单状态已改变，说明已支付
					const isPaid = res.data.paid === 1 || res.data.pay_price === 0 || res.data.status === 1;
					return {
						paid: isPaid,
		...
		async goPay(){
			...
			// #ifdef H5
			// H5模式下先检查订单状态，防止重复支付
			if (that.fromType === 'order' && that.orderId) {
				const statusRes = await this.checkOrderStatus();
		...
		handleH5Pay(payUrl, backUrl) {
			...
			// #ifdef H5
			// H5环境:直接跳转到支付页面,支付完成后会自动跳转到returnUrl(backUrl)
			console.log('H5环境:直接跳转到支付页面');
