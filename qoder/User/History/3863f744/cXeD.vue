...
export default {
  ...
  methods: {
    getOrderStatusList() {
      ...
      // 按照指定顺序添加状态
      // 1. 待审核
      const status1 = originalList.find(item => item.value === '1')
      if (status1) newList.push(status1)
      
      // 2. 未处理
      const status2 = originalList.find(item => item.value === '2')
      if (status2) newList.push(status2)
      
      // 3. 已受理 (value 7) - 移动到这里
      const status7 = originalList.find(item => item.value === '7')
      if (status7) newList.push(status7)
      
      // 4. 部分发货
      const status3 = originalList.find(item => item.value === '3')
      if (status3) newList.push(status3)
      
      // 5. 已发货
      const status4 = originalList.find(item => item.value === '4')
      if (status4) newList.push(status4)
      
      // 6. 已结束
      const status5 = originalList.find(item => item.value === '5')
      if (status5) newList.push(status5)
      
      // 7. 已取消
      const status6 = originalList.find(item => item.value === '6')
      if (status6) newList.push(status6)
