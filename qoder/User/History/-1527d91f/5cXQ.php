...
// 在事件触发前更新 refunded_price
    ...
    private function checkPlatformSubOrderExists(array $order, string $platformSubOrderId): bool
    {
        try {
            ...
            // 另一种检查方式：查询分账流水记录
            /** @var \app\services\pay\WechatTransferOrderServices $transferOrderServices */
            $transferOrderServices = app()->make(\app\services\pay\WechatTransferOrderServices::class);
            
            // 检查是否存在平台划付记录
            $platformTransferExists = $transferOrderServices->checkTransferExists(
                $order['order_id'], 
                'platform'
            );

            return $platformTransferExists;
