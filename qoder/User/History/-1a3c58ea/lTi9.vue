...
export default {
	...
	methods: {
			...
			switch (status) {
				...
				case "WECHAT_H5_PAY":
					...
					if (h5JsConfig && h5JsConfig.status === 'UMS_H5_PAY') {
						...
						// #ifdef H5
						// 微信浏览器内使用JS-SDK支付(像小程序一样)
						if (this.$wechat && this.$wechat.isWeixin && this.$wechat.isWeixin()) {
							console.log('微信浏览器内使用JS-SDK支付，像小程序一样');
							this.wechatPayFun(res.data.result, backUrl);
						} else {
							// 非微信浏览器使用原生微信H5支付
		...
		wechatPayFun(data, backUrl){
			...
			// #endif
			// #ifdef H5
			this.$wechat.pay(data.jsConfig).then(res => {
